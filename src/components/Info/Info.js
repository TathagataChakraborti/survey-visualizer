...
function getChildren(node, taxonomy) {
    ...
    const children = lower_level_data.filter(
        child => child.parent === node.name
    );
...
const Paper = props => (
    <ContainedListItem
        ...
        <p className="paper">
            <strong>{props.paper.title}</strong>{' '}
            <em>by {props.paper.authors}</em> {props.paper.venue} (
            {props.paper.year})
        </p>
    </ContainedListItem>
);
        ...
        return (
            <Grid>
                <Column lg={6} md={4} sm={4}>
                    <Slider
                        labelText={
                            config.metadata.acronym + ' through the years'
                        }
                        hideTextInput
